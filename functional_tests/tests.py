...
class NewVisitorTest(StaticLiveServerTestCase):
    ...
    def setUp(self):
        ...
        self.browser.implicitly_wait(3)  # 添加等待时间
        real_server = os.environ.get('REAL_SERVER')
        if real_server:
            self.live_server_url = 'http://39.101.73.165'#'http://' + real_server
    ...
    def wait_for_row_in_list_table(self, row_text):
        ...
        while True:
            try:
                ...
                self.assertIn(row_text, [row.text for row in rows])  # 检查每行的文本内容
                return
